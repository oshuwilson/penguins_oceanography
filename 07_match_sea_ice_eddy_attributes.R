#---------------------------------------------------------------------------------
# Match eddy amplitude, age, maturity, and intensity to locations (sea ice eddies)
#---------------------------------------------------------------------------------
...
# choose case study (Adelie Penguins Pointe Geologie)
i <- 33
  ...
  # limit data to 2013-2021
  data <- data %>%
    filter(date >= as_date("2013-01-01") & date <= as_date("2021-12-31"))
  
  
  # 2. Match to eddy IDs
  
  # read in eddy data
  cyc <- readRDS("data/eddies/cyclonic_attributes_auger.RDS")
  anti <- readRDS("data/eddies/anticyclonic_attributes_auger.RDS")
  
  # get unique dates in the data
  dates <- unique(as_date(data$date)) %>% sort()
  
  # include dates within 1 day of dates
  dates <- c(dates - days(1), dates, dates + days(1)) %>% unique() %>% sort()
  ...
  for(j in 1:length(dates)){
    ...
    # if nrow(eddies_sub) == 0, skip to next date
    if(nrow(eddies_sub) == 0){
      next
    }
    
    # limit locations to this date
    data_sub <- data %>%
      filter(date == this.date | date == this.date - days(1) | date == this.date + days(1))
