...
# remove NAs in eddy_importance
data <- data %>% filter(eddy_importance != "")

# group by species and cpf 
data <- data %>%
  group_by(species, cpf, eddy_importance) %>%
  summarise(count = n())

# add blank values for each missing combination
species_levels <- unique(data$species)
cpf_levels <- unique(data$cpf)
eddy_levels <- unique(data$eddy_importance)
blank <- expand.grid(species = species_levels, cpf = cpf_levels, eddy_importance = eddy_levels) %>%
  mutate(count = 0)
missing <- blank %>%
  anti_join(data, by = c("species", "cpf", "eddy_importance"))
data <- rbind(data, missing)

# modify factors
data <- data %>%
  mutate(eddy_importance = as.factor(eddy_importance),
         species = as.factor(species)) %>%
  mutate(eddy_importance = fct_relevel(eddy_importance, "specialised", "uncertain", "unspecialised", "not encountered"))

# capitalise
levels(data$eddy_importance) <- c("Specialised", "Uncertain", "Unspecialised", "Not Encountered")
levels(data$species) <- c("Ad\u00E9lie", "Chinstrap", "Emperor", "King", "Macaroni")
  
# create greyscale legend for eddy_importance
test <- data %>% filter(species == "Ad\u00E9lie")
p1 <- ggplot(test, aes(x = eddy_importance, y = count, fill = eddy_importance)) + geom_bar(stat = "identity") +
  theme_classic() +
...
high <- data %>%
  filter(cpf == "high")
p2 <- ggplot(high, aes(x = species, y = count, fill = interaction(species, eddy_importance))) +
...
mid <- data %>%
  filter(cpf == "mid")
p3 <- ggplot(mid, aes(x = species, y = count, fill = interaction(species, eddy_importance))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.8, col = "grey20", lwd = 0.1) +
...
p4 <- ggplot(none, aes(x = species, y = count, fill = interaction(species, eddy_importance))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.8, col = "grey20", lwd = 0.1) +
