...
tracks <- trax %>% as.data.frame(geom = "XY") %>%
  ...
  filter(!individual_id %in% c("M1", "M2"))
...
# for clarity remove the year with the fewest tracks (n = 2)
edplots2 <- edplots2[-4]

# add legend
edplots2[[6]] <- eddy_legend

# plot years together with legend
...
dep_years <- dep_years[dep_years != 2017]
for(z in 1:length(dep_years)){
  ...
  fsle <- rast(paste0("exploration/results colonies/ratmanoff/fsle/fsle_ratmanoff_", year, ".nc"))
...
lens <- tracks %>%
  group_by(individual_id) %>%
  summarise(max_dist = max(dist_to_colony, na.rm = T),
            year = year(min(date)))
  group_by(year) %>%
  summarise(mean_d = mean(max_dist),
            sd_d = sd(max_dist),
            n = n())
lens

ggplot(lens, aes(x = as.factor(year), y = max_dist)) + 
  geom_violin()

m1 <- aov(max_dist ~ as.factor(year), data = lens[-25,]) # removing outlier with too much leverage
summary(m1)
TukeyHSD(m1)
